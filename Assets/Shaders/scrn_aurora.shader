...
Shader "SCRN_Aurora MatCap"
{
	...
	Subshader
	{
		...
		Pass
		{
			...
			#include "/Assets/Shaders/tanoise.cginc"
...
#define STACKADVANCE 4.0 //Was 1.3 with SCRN
			...
			*/
			...
			fixed4 aurora(fixed3 ro, fixed3 rd)
			{
				...
				[unroll]
				for(fixed i=0.;i<22;i+=STACKADVANCE)// I know, this is terrible but it's not as banding prone as lowering trinoise
				{
...
//#ifdef CNLOHR_AURORA_MOD
				...
				fixed3 rn = tanoise3_1d_fast(id);//hash33(id).xy;
				rn.xy = (rn.xy + rn.zz) / 2.;
			...
			fixed3 getSceneColor( in fixed3 ro, in fixed3 rd ) {
				...
				if( intersectPlane( ro, rd, -.01, refldist ) && refldist < 5000. ) {
					...
					fresnel = pow(1.0-abs(ndotr),5.)+0.2;
			...
			custom_type vertex_shader (fixed4 vertex : POSITION, fixed2 uv:TEXCOORD0, fixed3 normal : NORMAL)
			{
				...
				vs.sNormal = mul(unity_ObjectToWorld, -normal ); /* NOTE XXX CNL FLIP NORMAL BECUASE WE ARE FLIPPED */
				return vs;
			}
			...
			fixed4 pixel_shader (custom_type ps) : SV_TARGET
			{
				...
				return fixed4(getSceneColor( worldPosition + _Position, normalize( viewDirection + ps.sNormal*.2) ),1.0);
			}
			ENDCG
		}
	}
